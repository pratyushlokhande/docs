...
1. **Load**: Load your data sources into [`Document`](https://reference.langchain.com/python/langchain-core/documents/base/Document) objects.
2. **Split**: Use [text splitters](/oss/python/integrations/splitters) to break large `Document`s into smaller chunks. This is useful both for indexing data and passing it to a model, as large chunks are harder to search over and either do not fit in a model's finite context window or use more tokens than necessary.
3. **Embed**: [Embeddings](/oss/python/integrations/embeddings) models convert each chunk into a numeric vector that captures its meaning, enabling similarity search over your content.
4. **Store**: Use a [VectorStore](/oss/python/integrations/vectorstores) to index chunks and their embeddings for retrieval.
...
<Note>
    If you have completed the [semantic search tutorial](/oss/python/langchain/knowledge-base), you can use the function to execute a search from it and skip to [Retrieve and generate answers](#retrieve-content-and-generate-answers).
</Note>
...
Now that you have indexed the data, you will create an [agent](/oss/python/langchain/agents) that takes a user question, searches for documents relevant to that question, passes the retrieved documents and initial question to a model, and returns an answer.
...
The first RAG agent you will create is a minimal [agent](/oss/python/langchain/agents) with a tool that retrieves information.
...
In the [RAG agent](#create-a-rag-agent) you created, you allow the LLM to use its discretion in generating a [tool call](/oss/python/langchain/models#tool-calling) to help answer user queries. This is a good general-purpose solution, but comes with some trade-offs:
