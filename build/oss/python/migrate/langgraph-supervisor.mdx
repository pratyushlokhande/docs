...
Tool-wrapped subagents work for most interrupt and resume flows. Use a custom [`StateGraph`](https://reference.langchain.com/python/langgraph/graph/state/StateGraph) with the subagent as a [node](/oss/python/langgraph/use-subgraphs#call-a-subgraph-inside-a-node) when your external callback path needs to [inspect nested graph state](/oss/python/langgraph/use-subgraphs#view-subgraph-state) while paused.

Because subagents are called inside tool functions, LangGraph cannot statically discover them. This means [`get_state`](https://reference.langchain.com/python/langgraph/graphs/#langgraph.graph.state.CompiledStateGraph.get_state) with `subgraphs=True` does not return subagent state for tool-wrapped agents. If your callback handler needs to read nested state (for example, to look up a `job_id` stored in a subagent's checkpoint), invoke the subagent from a node function in a custom graph instead.
